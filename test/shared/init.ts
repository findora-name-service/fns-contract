...
export async function v2Fixture(): Promise<V2Fixture> {
  const [ aUser, bUser, cUser, dUser, eUser, fUser, gUser ] = await ethers.getSigners();
  // deploy tokens
  ...
  await nameRegistrar.setSecondClaimTime(getAddDaysTime(7), getAddDaysTime(9));
  await nameRegistrar.setPublicTime(getAddDaysTime(11));
